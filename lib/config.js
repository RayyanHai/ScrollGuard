...
self.SG_CONFIG = {
  ...
  // Password
  // -------------------------------------------------------------------------
  // No default. First-time users are forced through a setup flow in the popup
  // that writes their chosen password into `userConfig.password`. Until that
  // happens, the block overlay shows a "set up in the popup" message instead
  // of an unlock field, and TRY_UNLOCK messages are rejected.
  //
  // Stored in plaintext because this is a self-imposed friction layer, not a
  // security boundary — anyone with chrome://extensions devtools access can
  // read it. The point is to make bypass annoying enough that you reflect.
  password: null,
};
